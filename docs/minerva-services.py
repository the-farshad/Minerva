...
def _write_cookies_override(here, cookies_path):
    ...
    Compose auto-loads override files, so this composes with whatever
    the upstream docker-compose.yml says.
    """
    target = here / "docker-compose.override.yml"
    target.write_text(
        "# Auto-generated by `minerva-services.py up` — safe to delete.\n"
        "# Bind-mounts your host cookies.txt so yt-dlp can authenticate.\n"
        "services:\n"
        "  minerva-services:\n"
        "    volumes:\n"
...
def _cmd_up(browser):
    ...
    _write_cookies_override(here, cookies_file)
